...
def _make_bottle_neck_block(in_features, out_features, drop_rate=0.0):
        ...
        layers=[
            nn.BatchNorm2d(in_features),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_features, inner_features, 
        ...
        layers.extend([
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_features, out_features, kernel_size=3, padding=1, bias=False)
        ])
        return nn.Sequential(*layers)
...
class DenseNetBlock(nn.Module):
    def __init__(self, 
        ...
        for i in range(num_layers):
            module = block_function(in_features + i * growth_rate, growth_rate, drop_rate)
    ...
    def __init__(self, 
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                transition_block = _make_transition(
