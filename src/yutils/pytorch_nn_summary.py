...
class PytorchModelSummaryCollector:
    ...
    def __call__(self, module, module_in, module_out):
        ...
        if isinstance(module_out, (list, tuple)):
            summary["output_shape"] = [[-1] + list(o.size())[1:] for o in module_out]
        else:
    ...
    def __repr__(self):
        ...
        total_input_size = np.prod(self.input_size) * batch_size * 4.0 / (1024 ** 2.0)
        # x2 for gradients
        total_output_size = 2.0 * self.total_output * 4.0 / (1024 ** 2.0)
        total_params_size = self.total_params * 4.0 / (1024 ** 2.0)
